...
public class FluidsHelperImpl {
    ...
    public static FluidOffer getFluidInTank(Level level, BlockPos pos, Direction dir, BlockEntity source) {
        ...
        if (handler != null) {
            ...
            //simulate all possible amounts: a tank may only allow certain increments
            for (int i = 1; i <= 4; i++) {
                int toDrain = bottlesToMB(i);
                int drained;
                try (Transaction t = Transaction.openRoot()) {
                    //never committed, so this is a simulation
    ...
    /**
     * The transfer API needs a resource to extract, unlike the old amount-only drain, so we take whatever
     * the handler currently holds.
     */
    @Nullable
