...
public interface IFabricMenuType<T> {
    ...
    static <T extends AbstractContainerMenu> MenuType<T> create(Factory<T> factory) {
        // fabric-api's extended screen handler API is gone since 26.1: the extra data (if any) was
        // stashed by ClientBoundOpenExtendedMenuMessage, sent just before the open screen packet
