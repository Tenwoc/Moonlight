...
public class ClientHelperImpl {
    ...
    /**
     * Fabric has no level aware collectParts, its models go through the renderer pipeline instead.
     * QuadEmitter takes that path directly, this is only the fallback for plain quad access.
     */
    ...
    public static void addRenderPipelineRegistration(Consumer<ClientHelper.RenderPipelineEvent> eventListener) {
        Moonlight.assertInitPhase();
        //fabric api has no equivalent event. Registering just means the pipeline gets precompiled
        //and validated on resource reload instead of failing at the first draw call
    ...
    public static void addPictureInPictureRendererRegistration(Consumer<ClientHelper.PictureInPictureEvent> eventListener) {
        Moonlight.assertInitPhase();
        //fabric keys the renderer off its own getRenderStateClass, so the class we are handed here is redundant
    ...
    public static void addSpecialModelRegistration(Consumer<ClientHelper.SpecialModelEvent> eventListener) {
        //fabric-api has no event for this, the vanilla id mapper is public and late bound anyway
