...
public abstract class DataMapBridge<T, O> extends SimplePreparableReloadListener<List<JsonElement>> {
    ...
    //runs before entries are re-applied on a (re)load, for bridges that accumulate state
    ...
    protected static class BurnTimes extends DataMapBridge<BurnTimes.FurnaceFuel, Item> {
        ...
        // Fabric's FuelRegistry is gone: FuelValues are immutable and rebuilt via an event, so we
        // stash the latest parsed values here and feed them to the builder on each rebuild.
        // The server rebuilds FuelValues right after every datapack (re)load, so ordering works out.
