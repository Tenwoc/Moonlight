...
public class MoonlightClient {
        ...
        protected void regenerateDynamicAssets(Consumer<ResourceGenTask> executor) {
            ...
            if (fixShade != ClientConfigs.ShadeFix.FALSE) {
                executor.accept((manager, sink) -> {
                    // vanilla's own light.glsl with the power and ambient term swapped out when it sees
                    // our light directions. dot(NEW_L_0, NEW_L_1) is 0.2039, which no vanilla pair hits
    ...
    // captured before we overwrite them so turning the fix back off restores whatever vanilla had
    private static Vector3f @Nullable [] vanillaDiffuseLights = null;

    // Swaps the diffuse light directions the game shades entities and items with. The level entries are plain mutable
    // statics, but the gui item ones are baked into the lighting ubo when Lighting is constructed, so those have to be
    // pushed again by hand.
