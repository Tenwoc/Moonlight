...
public class MLRenderTypes {
    ...
    // Vanilla only ever binds atlas textures with their own sampler, which never mipmaps. This one does, for the mipped
    // variants below
    ...
    // Paints the shape of a texture with the vertex color, ignoring the texture's own colors. Uses vanilla's text vertex
    // shader, so it wants POSITION_COLOR_TEX_LIGHTMAP quads
    ...
    // Vanilla only has opaque and translucent particle pipelines. Same shaders, additive blending
