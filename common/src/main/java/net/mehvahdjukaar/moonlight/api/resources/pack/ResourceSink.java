...
public class ResourceSink {
    ...
    /**
     * Adds an item geometry model (models/item). On its own this does nothing:
     * an item only shows it once an item model definition points at it
     */
    ...
    /**
     * Adds an item model definition (items), the tree that decides which model a stack of the
     * given item renders with. Location is the item id, not a model path
     */
