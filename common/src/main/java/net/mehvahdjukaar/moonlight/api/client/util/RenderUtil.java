...
public class RenderUtil {
    ...
    /**
     * Draws a block state at the current pose, the way a falling block or a piston head is drawn: full block
     * lighting, ambient occlusion and biome tints, all resolved against the given level position.
     * <p>
     * Replaces the old renderBlock that tesselated a baked model into a buffer. 26.1 renders through the
     * submit pipeline, so a renderer hands the block to the collector and the pipeline batches it.
     *
     * @param pos           position the block is lit and tinted as if it were at
     * @param randomSeedPos position its model randomization is seeded from (usually the same)
     */
    ...
    /**
     * Draws an arbitrary block model at the current pose with an explicit light value. The weaker counterpart to
     * submitBlock: no level lighting, tints or ambient occlusion, just the model's own geometry.
     * Pass the level/pos/state when the model is level aware (mimic blocks, connected textures), null otherwise.
     */
    ...
    /**
     * Draws an item in a GUI under an arbitrary 3D transform, which the flat GUI pose can't express.
     * Thin wrapper over AnimatedGuiItem, see there for the cost of doing this.
     */
    ...
    public static void renderSprite(PoseStack.Pose pose, VertexConsumer vertexBuilder, int light,
        ...
        // sprites carry their own padding now, so the uvs need no extra inset (uvShrinkRatio is gone)
