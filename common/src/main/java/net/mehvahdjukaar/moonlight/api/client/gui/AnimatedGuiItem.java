...
/**
 * Draws an item in a GUI with an arbitrary 3D transform and an optional tint, neither of which
 * GuiGraphicsExtractor.item can do: the GUI pose is 2D, and items normally go through the shared item atlas.
 * <p>
 * This costs an offscreen render target per distinct item on screen (both loaders pool and reuse them across frames),
 * so use it only for the few items that actually need it and fall back to graphics.item for the rest.
 * register must be called once during client setup.
 */
public final class AnimatedGuiItem {

    /**
     * Post-multiplied into each of the item's model layers, so it lands <em>inside</em> the model's own display
     * transform - the same place the transform used to go when items were drawn through ItemRenderer.
     * That is what makes a rotation spin the model about its own axis instead of swinging it around the screen's.
     *
     * @param blockModel whether the item renders as a 3D block rather than a flat sprite
     */
    @FunctionalInterface
    ...
    /**
     * Draws stack size pixels wide at (x, y). Pass -1 as the tint to leave
     * the colors alone.
     */
    public static void submit(GuiGraphicsExtractor graphics, ItemStack stack, int x, int y, int size,
        ...
        // the render target is twice the icon, centered on it, so a spinning block (sqrt(3) wider across its diagonal
        // than its side) has room instead of clipping against the texture edge
        int pad = size / 2;
    ...
    public static class Renderer extends PictureInPictureRenderer<State> {
        ...
        protected void blitTexture(State state, GuiRenderState guiRenderState) {
            ...
            // same blit as the base class, with the tint applied. The item is drawn premultiplied, so a grey tint
            // darkens it evenly and leaves the transparent parts alone
