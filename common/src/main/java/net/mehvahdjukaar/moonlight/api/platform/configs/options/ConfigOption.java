 ...
 * value of the same type and extend SimpleConfigOption; the compound kinds (range/vec3/json) sit on top of
...
public abstract class ConfigOption<T> extends ConfigNode {
    ...
    public static class BooleanValue extends SimpleConfigOption<Boolean> {
        // a "feature" boolean renders as the check/cross toggle instead of the plain ON/OFF button. A category gate is drawn
    ...
    /** A double drawn as a slider instead of a text field. See IntSliderValue. */
    ...
    /** A [0, 1] double drawn as a slider that displays a percentage. */
    ...
    /** Integer counterpart of Vec3Value. */
    ...
    /**
     * A list of strings, edited on a dedicated add/remove page. With options present each entry is picked
     ...
     * page. The screen sees a String; apply parses it back into a JsonElement.
     */
    ...
    /**
     * A codec-backed object value that can be edited, since it carries a SchemaCodec: its row opens a form
