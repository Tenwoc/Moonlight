 ...
 * paletted_permutations atlas sprite source. Those repaletted sprites are virtual: they have
 * no backing png and are baked straight into an atlas at stitch time, so SpriteUtils.readImage
 * cannot find them. Their three ingredients (grayscale base, palette key, permutation palette) are however
 * real resources, so we can re-run the exact vanilla recipe here and hand back a normal TextureImage
 * that the recoloring pipeline can consume like any other texture.
 * <p>
 * The atlas definitions are scanned once per ResourceManager (i.e. once per reload, and only the
...
public final class PalettedPermutationsHelper {
     ...
     *
     * @param base        grayscale base texture id (sprite id form, no textures/ prefix / .png)
     ...
     *
     * @param spriteId the sprite id (relative texture path, no textures/ prefix or .png)
     * @return a freshly baked TextureImage, or null if this id is not a known permutation
    ...
    /**
     * Drops the cached index. Called on client resource reload (next to TextureCache.clear()) so nothing is
     * retained between reloads. Correctness across the reload itself is guaranteed independently by the
     * manager-identity check in getIndex, since this cache is consumed at generation time (earlier).
     */
