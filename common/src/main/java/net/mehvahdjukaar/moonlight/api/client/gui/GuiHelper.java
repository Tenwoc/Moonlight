...
public final class GuiHelper {
    ...
    /**
     * A left-to-right gradient, which GuiGraphicsExtractor.fillGradient can't do. Drawn as 1px columns, so
     * keep the span narrow (edge fades, highlights) rather than filling whole screens with it.
     */
    ...
    /** Vertical counterpart to renderSeparator. Vanilla's sprites are horizontal, so this one is drawn. */
    ...
    /** The tiling list background over a scroll panel, as AbstractSelectionList draws it. */
    ...
    /** The bottom inner-shadow strip, as AbstractSelectionList draws it. */
    ...
    /**
     * The placeholder mod icon: a dark square with the mod's capital initial, or gearIcon when the name is
    ...
    /** A number as a text field shows it: no trailing .0 when it happens to be whole. */
     ...
     * AbstractWidget.extractScrollingStringOverContents does, which needs a widget owner.
     */
    ...
    /** Like renderScrollingText but centered while the text fits, only marqueeing once it overflows. */
